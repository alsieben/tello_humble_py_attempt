...
# heavily stolen from https://github.com/tentone/tello-ros2/blob/main/workspace/src/tello/tello/node.py

class TelloCoreNode(Node):
    """Core Tello node for accessing Tello drone functions via the ROS 2 interface"""
    def __init__(self):
        super().__init__('tello_core_node')

        # declare parameters
        self.declare_parameter('connect_timeout', 10.0) # connection timeout time
        self.declare_parameter('tello_ip', '192.168.10.1') # drone ip
        self.declare_parameter('tf_base', 'map') # name of base frame
        ...
        if len(self.camera_info_file) == 0:
            share_directory = ament_index_python.get_package_share_directory('tello_humble_py')
